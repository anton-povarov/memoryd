(() => {
  ...
  function renderMemoryList() {
    ...
    for (const memory of memories) {
      ...
      row.setAttribute("aria-pressed", memory.id === state.selectedID ? "true" : "false")
  ...
  function renderDetail(detail) {
    ...
    const contentURL = detail.content_url || `${apiBase}/memories/${memory.id}/content`
    ...
    addMetadata("Byte size", formatBytes(context.byte_size ?? memory.byte_size))
    addMetadata("Imported", formatDate(memory.imported_at, "long"))
    addMetadata("Original created", formatDate(context.filesystem_created_at, "long"))
    addMetadata("Original modified", formatDate(context.filesystem_modified_at, "long"))
  ...
  async function renderPreview(memory, contentURL) {
    ...
    try {
      ...
      if (state.selectedID !== memory.id) return
      renderPreviewPlaceholder(memory, `The original is still available to download. ${error.message}`)
    }
  }
  ...
  function removeSelectedFile(itemID) {
    ...
    state.selectedFiles = state.selectedFiles.filter((item) => item.id !== itemID)
    renderFileQueue()
  }
  ...
  function queueStatus(item) {
    switch (item.status) {
      ...
      case "duplicate": return "Already stored"
  ...
  function renderFileQueue() {
    ...
    const readyCount = state.selectedFiles.filter((item) => item.status === "ready").length
    elements.clearFiles.disabled = state.importing
    elements.addMoreFiles.hidden = state.importing || state.importComplete
    elements.submitImport.disabled = state.importing || (!state.importComplete && readyCount === 0)
  ...
  function uploadMemory(file, onProgress) {
    ...
    return new Promise((resolve, reject) => {
      ...
      request.addEventListener("error", () => {
        reject({ status: 0, payload: null, message: "Could not reach the Vault." })
      })
      request.send(body)
    })
  }
  ...
  async function handleImport(event) {
    ...
    const readyItems = state.selectedFiles.filter((item) => item.status === "ready")
    ...
    for (let index = 0; index < readyItems.length; index += 1) {
      ...
      try {
        ...
        state.memories = [memory, ...state.memories.filter((entry) => entry.id !== memory.id)]
      } catch (error) {
        if (error.status === 409) {
          item.status = "duplicate"
    ...
    const duplicates = state.selectedFiles.filter((item) => item.status === "duplicate").length
  ...
  elements.loadMore.addEventListener("click", () => loadMemories({ append: true }))
